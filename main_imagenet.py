...
from NAPAVQ_imagenet import NAPAVQ
from backbone.ResNet_imagenet import resnet18_ImageNet
...
parser.add_argument('--custom_name', default='new-code-imagenet-1', type=str,
                    help='custom name for each experiment')
parser.add_argument('--base_model', default='new-code-imagenet-1', type=str, help='base_model for task 0')
...
def main():
    ...
    data_manager = DataManagerImagenet(shuffle=args.shuffle, seed=args.seed)
    ...
    for current_task in range(args.task_num + 1):
        ...
        for i in range(current_task + 1):
            ...
            for setp, (imgs, labels) in enumerate(test_loader):
                ...
                with torch.no_grad():
                    features = model.module.feature(imgs)
